...
def get_color_for_state(state: str) -> tuple:
    ...
    color_map = {
        ...
        "first": FIRST_HALF_COLOR,
        "second": SECOND_HALF_COLOR,
        "mfirst": MFIRST_HALF_COLOR,
        "msecond": MSECOND_HALF_COLOR
    }
...
def draw_split_cell(draw, x0: int, y0: int, x1: int, y1: int, state: str, prev_state: str, next_state: str, outline_color: tuple):
    ...
    if state == "no":
        ...
        if next_state == "no":
            ...
            right_color = FIRST_HALF_COLOR if next_state == "first" else MFIRST_HALF_COLOR
        ...
        if prev_state == "no":
            ...
            left_color = SECOND_HALF_COLOR if prev_state == "second" else MSECOND_HALF_COLOR
        ...
        right_color = MSECOND_HALF_COLOR
        # Перевіряємо стан попередньої години
        if prev_state == "no":
            left_color = OUTAGE_COLOR
        elif prev_state == "maybe":
